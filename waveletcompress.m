...
%Decomposition is not nessesery

% % [thr,nkeep] = wdcbm2(c,s,1.5,3*prod(s(1,:))); % %
% [compressed_image,TREED,comp_ratio,PERFL2]
% =wpdencmp(thr,'s',n,'haar','threshold',5,1); % %  disp('compression ratio
% in percentage'); % %  disp(comp_ratio); % %   re_ima1 =
% waverec2(c,s,'haar'); % %  re_ima=uint8(re_ima1); % %   subplot(1,3,1); %
% %  imshow(input_image1); % %  title('i/p image'); % %  subplot(1,3,2); %
% %  imshow(compressed_image); % %  title('compressed image'); % %
% subplot(1,3,3); % %  imshow(re_ima); % %  title('reconstructed image');
end